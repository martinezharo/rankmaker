...
## Database migrations
* If a database migration is safe for the version deployed before the current PR—meaning the old code and the migrated database can coexist without issues—do not be afraid to run it when useful to validate the PR against the real database.
## Maintainability
