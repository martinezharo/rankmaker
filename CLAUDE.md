...
- **Large i18n changes: English-first workflow.** When a task involves adding or rewriting many user-visible strings, implement everything in `en.ts` only and leave the other locale files untouched (the missing keys fall back to English automatically — the build passes without any placeholder edits). Once the English copy is complete and the feature works, generate a handoff file at `translations-pending/YYYY-MM-DD-<feature>.md` containing: (1) a brief description of the feature, (2) the full list of new/changed keys with their English values, and (3) explicit instructions for a translator AI to add real translations for those keys to `es.ts`, `fr.ts`, `de.ts`, `ms.ts`, and `zh.ts`.
