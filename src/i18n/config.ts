 ...
 * (`/template/x`), translated locales are prefixed (for example,
 * `/es/template/x`, `/fr/template/x`, and `/pt/template/x`). See `astro.config.mjs` (`i18n` block) and
 * `src/middleware.ts` (the prefix → rewrite step that sets `locals.locale`).
 */
export const locales = ['en', 'es', 'fr', 'zh', 'ms', 'de', 'pt'] as const;
