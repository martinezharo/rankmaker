...
describe('computeCanvasHeight', () => {
	// HEADER_H(110) + PODIUM_H(460) + FOOTER_H(60) + PAD(50) = 680 baseline.
	it('has no rest band for 3 or fewer items', () => {
		expect(computeCanvasHeight(1)).toBe(680);
		expect(computeCanvasHeight(3)).toBe(680);
	});

	it('adds one 2-column row band for items 4–5', () => {
		// 1 row → ROW_H(100) + label band(60) = 160.
		expect(computeCanvasHeight(4)).toBe(840);
		expect(computeCanvasHeight(5)).toBe(840);
	});

	it('grows by a row every two extra items', () => {
		// 6 items → 3 rest → 2 rows → 2*100 + 60 = 260.
		expect(computeCanvasHeight(6)).toBe(940);
	});
});
